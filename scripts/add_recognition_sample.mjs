...
const data=JSON.parse(await fs.readFile(path.join(root,'dist','data.json'),'utf8'));
const names=new Set(Object.values(data.modes).flatMap(rows=>rows.map(row=>row.name)));
for(const side of ['mine','opp'])if(new Set(labels[side]).size!==6||labels[side].some(name=>!names.has(name)))throw Error('each side must contain six unique current Pokemon names');
const dbPath=path.join(root,'dist','recognition-db.json');
const db=JSON.parse(await fs.readFile(dbPath,'utf8'));
if(db.schemaVersion!==3||!Array.isArray(db.references))throw Error('expected v16 recognition database schema 3');
...
if(!id||left[0]>=left[1]||right[0]>=right[1]||rows.some((y,i)=>i>0&&y<=rows[i-1]))throw Error('sample id, increasing rows and positive box widths are required');
...
db.updatedAt=new Date().toISOString().slice(0,10);db.references=[...db.references.filter(r=>!r.src?.startsWith(`recognition/${id}-`)),...references];
