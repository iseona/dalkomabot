...
const prefix=process.argv.includes('--wide')?'wide-':process.argv.includes('--cross')?'cross-':'';
...
try{
 ...
 const slots=await page.evaluate(async({prefix,scaled,erase})=>{const {readLocalPartyText}=await import('/dist/local-party-text.mjs');const files=await Promise.all(['ability','stats'].map(async type=>{let blob=await(await fetch(`/tests/fixtures/recognition/${prefix}party-${type}.${prefix==='wide-'?'png':'jpg'}`)).blob();if(scaled||erase){const b=await createImageBitmap(blob),c=document.createElement('canvas');c.width=scaled?1280:b.width;c.height=scaled?720:b.height;c.getContext('2d').drawImage(b,0,0,c.width,c.height);if(erase){const {detectedPartyCardRects}=await import('/dist/recognition-contact-sheet.mjs');const r=detectedPartyCardRects(c)[0],ctx=c.getContext('2d');ctx.fillStyle='#9a85c9';const a=type==='stats'?[.285,.30,.08,.20]:[.665,.095,.29,.17];ctx.fillRect(r.x+r.width*a[0],r.y+r.height*a[1],r.width*a[2],r.height*a[3]);}blob=await new Promise(r=>c.toBlob(r,'image/jpeg',.85));b.close();}return new File([blob],type+'.jpg',{type:'image/jpeg'});}));return readLocalPartyText(files);},{prefix,scaled:process.argv.includes('--720p'),erase:process.argv.includes('--erase')});
 await writeFile(path.join(root,`reports/${prefix}local-party-reader${process.argv.includes('--720p')?'-720p':''}${process.argv.includes('--erase')?'-erased':''}.json`),JSON.stringify(slots,null,2));
 const expected=prefix?JSON.parse(await readFile(`tests/fixtures/recognition/${prefix}party-expected.json`)):JSON.parse(await readFile('tests/fixtures/recognition/manifest.json')).party;
