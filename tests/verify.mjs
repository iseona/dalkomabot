...
import {draft,makeSet,validSet,validEV,recommend,empiricalCoverage,calculateStats,damageRolls,classifyKO,battleCalculation,strongestStatMove,damageScenarioResults,speedVariants,teamDefenseMatrix,coverageDelta} from '../dist/engine.mjs';
...
import {findPokemonMatches} from '../dist/pokemon-search.mjs';
import {loadSavedTeams,partyExport} from '../dist/party-storage.mjs';
...
assert.equal(commandPayload.length,10);assert.equal(new Set(commandPayload.map(command=>command.name)).size,10);assert(commandPayload.every(command=>command.type===1));
...
{const rows=D.modes.single,matrix=teamDefenseMatrix(rows,['한카리아스']);assert.equal(matrix.length,18);assert.equal(matrix.find(x=>x.type==='얼음').cells[0].value,4);const meta=rows.find(x=>x.name==='메타그로스'),delta=coverageDelta(meta,rows,['한카리아스']);assert(delta.repaired.includes('얼음'));assert(delta.afterScore>delta.beforeScore);const full=coverageDelta(meta,rows,rows.slice(0,6).map(p=>p.name));assert(full.replaced);assert.equal(full.members.length,6);assert(full.members.includes(meta.name))}
...
{const rows=D.modes.single;for(const query of ['나인테일','알로라 나인테일','나인테일 알로라폼']){const matches=findPokemonMatches(rows,query);assert(matches.some(p=>p.name==='알로라 나인테일'),query)}assert.equal(findPokemonMatches(rows,'나인테일')[0].name,'나인테일');const target={innerHTML:''},matches=findPokemonMatches(rows,'나인테일 알로라폼');renderPartyPokemonMatches(target,matches,2,escapeHtml);assert(target.innerHTML.includes('data-ocrcandidate="2"'));assert(target.innerHTML.includes('data-name="알로라 나인테일"'));renderLeadPokemonMatches(target,matches,'opp',4,escapeHtml);assert(target.innerHTML.includes('data-leadside="opp"'));assert(target.innerHTML.includes('data-leadindex="4"'));assert(target.innerHTML.includes('data-leadname="알로라 나인테일"'))}
{const memory=new Map([['champions-party-v1',JSON.stringify({single:[makeSet(D.modes.single[0]),{name:'없는포켓몬'},null,null,null,null],double:Array(6).fill(null)})]]),storage={getItem:key=>memory.get(key)||null,setItem:(key,value)=>memory.set(key,value)};const restored=loadSavedTeams(storage,(set,mode)=>validSet(set,D.modes[mode],D.master));assert(restored.repaired);assert(restored.teams.single[0]);assert.equal(restored.teams.single[1],null);assert(memory.has('champions-party-v1-backup'));assert.equal(partyExport('single',D.date,restored.teams.single).team[0].name,restored.teams.single[0].name)}
{const set=makeSet(D.modes.single[0]),storage={getItem:()=>null,setItem:()=>{}},added=addSetsToLibrary([], [set],'single',1);assert.equal(added.length,1);assert.equal(loadSetLibrary(storage,()=>true).length,0);assert.equal(removeSetFromLibrary(added,added[0].id).length,0)}
...
assert(code.includes("from './ai-recognition.mjs?v=20'"));assert(code.includes("kind:'party'"));
...
assert(uiTemplatesCode.includes('data-tab="lead"'));assert(code.includes("kind:'lead'"));assert(code.includes('mergeLeadRecognition(result.sides'));assert(code.includes("mode==='single'?3:4"));assert(code.includes('maxDamage(source,target'));
assert(code.includes("recognizeImages({files:[input],mode:inputMode,kind:'lead',leadRectDetector:detectedLeadRects})"));assert(code.includes('AI 인식 완료'));
assert(code.includes('if(!result&&useAI)'));assert(code.includes('recognizeLeadOnDevice(input)'));
...
assert(code.includes("fetch('champions-icon-map.json')"));assert(!code.includes("fetch('sprite-map.json')"));assert(!code.includes("fetch('sprite-home-map.json')"));assert(code.includes("fetch('recognition-db.json')"));assert(recognitionCode.includes('fineSilhouette'));assert(code.includes('saveLearnedReferences'));
...
const html=fs.readFileSync(root+'/dist/index.html','utf8'),manifest=JSON.parse(fs.readFileSync(root+'/dist/manifest.webmanifest')),version=JSON.parse(fs.readFileSync(root+'/dist/version.json'));assert(html.includes('<title>포챔스 달콤아 봇</title>'));assert(html.includes('assets/dalkoma-logo-192.png'));assert(html.includes('dalkoma-favicon.png?v=16'));assert(html.includes('<strong class="appversion">v22</strong>'));assert(html.includes('id="imageFile" accept="image/png,image/jpeg,image/webp" multiple'));assert(!html.includes('id="screenType"'));assert(html.includes('style.css?v=22'));assert(html.includes('app.js?v=22'));assert.equal(manifest.name,'포챔스 달콤아 봇');assert.equal(version.version,'v22');assert.equal(version.builtAt,'2026-09-14');assert.equal(version.name,'포챔스 달콤아 봇');for(const icon of ['dalkoma-favicon.png','dalkoma-apple-touch.png','dalkoma-logo-192.png','dalkoma-logo-512.png'])assert(fs.statSync(root+'/dist/assets/'+icon).size>100);assert(!code.includes("source:'PokeAPI 보조'"));
for(const section of ['builder','meta','import','ranked'])assert(html.includes(`id="${section}"`));assert(uiTemplatesCode.includes('id="coverage"'));assert(uiTemplatesCode.includes('id="library"'));assert(uiTemplatesCode.includes('id="lead"'));assert(uiTemplatesCode.includes('id="calculators"'));assert(code.includes("['builder','meta','import','ranked','coverage','library','lead','calculators']"));
for(const moduleName of ['recognition.mjs','ai-recognition.mjs','recognition-merge.mjs','ui.mjs','calculator.mjs'])assert(code.includes(`./${moduleName}?v=20`));for(const moduleName of ['engine.mjs','ui-templates.mjs'])assert(code.includes(`./${moduleName}?v=22`));
const deploy=fs.readFileSync(root+'/aws/deploy.py','utf8'),recognizer=fs.readFileSync(root+'/aws/recognizer.py','utf8');assert(deploy.includes("'no-cache, no-store, must-revalidate'"));assert(deploy.includes("'invalidation-completed'"));assert(deploy.includes('OPENAI_API_KEY'));assert(deploy.includes('MaxDailyRecognitionRequests'));assert(recognizer.includes("'store':False"));assert(recognizer.includes("'max_output_tokens':1600"));assert(recognizer.includes('consume_quota()'));
...
try{
 ...
 const originalFetch=globalThis.fetch,refreshed=structuredClone(O);refreshed.modes.single.updatedAt='future refresh accepted';refreshed.modes.single.season='M-6';refreshed.modes.double.season='M-6';process.env.APP_URL='https://example.invalid/app/';globalThis.fetch=async()=>({ok:true,json:async()=>refreshed});
 try{const refreshedModule=await import(pathToFileURL(temp+'/handler.mjs').href+'?refresh=future');const result=JSON.parse((await refreshedModule.handler(event({type:2,data:{name:'메타',options:[]}}))).body);assert(result.data.content.includes('future refresh accepted'))}finally{globalThis.fetch=originalFetch;delete process.env.APP_URL}
