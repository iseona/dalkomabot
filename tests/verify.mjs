...
assert(code.includes("from './ai-recognition.mjs?v=17'"));assert(code.includes("kind:'party'"));
...
const html=fs.readFileSync(root+'/dist/index.html','utf8'),manifest=JSON.parse(fs.readFileSync(root+'/dist/manifest.webmanifest')),version=JSON.parse(fs.readFileSync(root+'/dist/version.json'));assert(html.includes('<title>포챔스 달콤아 봇</title>'));assert(html.includes('assets/dalkoma-logo-192.png'));assert(html.includes('dalkoma-favicon.png?v=16'));assert(html.includes('<strong class="appversion">v17</strong>'));assert(html.includes('id="imageFile" accept="image/png,image/jpeg,image/webp" multiple'));assert(!html.includes('id="screenType"'));assert(html.includes('style.css?v=17'));assert(html.includes('app.js?v=17'));assert.equal(manifest.name,'포챔스 달콤아 봇');assert.equal(version.version,'v17');assert.equal(version.builtAt,'2026-09-09');assert.equal(version.name,'포챔스 달콤아 봇');for(const icon of ['dalkoma-favicon.png','dalkoma-apple-touch.png','dalkoma-logo-192.png','dalkoma-logo-512.png'])assert(fs.statSync(root+'/dist/assets/'+icon).size>100);assert(!code.includes("source:'PokeAPI 보조'"));
for(const section of ['builder','meta','import','ranked'])assert(html.includes(`id="${section}"`));assert(uiTemplatesCode.includes('id="lead"'));assert(uiTemplatesCode.includes('id="calculators"'));assert(code.includes("['builder','meta','import','ranked','lead','calculators']"));
for(const moduleName of ['engine.mjs','recognition.mjs','ai-recognition.mjs','ui-templates.mjs','ui.mjs','calculator.mjs'])assert(code.includes(`./${moduleName}?v=17`));
