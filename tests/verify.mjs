...
let code=fs.readFileSync(root+'/dist/app.js','utf8'),parseStats=new Function(code.match(/function parseStats\(text\).*\n/)[0]+';return parseStats')();
...
assert(code.includes("vals=parseStats(text)"));
...
assert(code.includes('data-tab="lead"'));assert(code.includes("panelLayout(img,'mine')"));assert(code.includes('SHINY_SPRITES'));assert(code.includes("mode==='single'?3:4"));assert(code.includes('maxDamage(source,target'));
assert(code.includes('const regions='));assert(code.includes('uniqueVisualRankings'));assert(code.includes("distances[0]*.72"));
assert(code.includes('data-tab="calculators"'));assert(code.includes('function runCalculator()'));assert(code.includes('function runPowerSweep()'));assert(code.includes('function runSpeedCompare()'));assert(code.includes('data-leadmatches'));assert(!code.includes('레벨 50'));
assert(code.includes("fetch('sprite-home-map.json')"));assert(code.includes("fetch('sprite-home-shiny-map.json')"));assert(code.includes("fetch('recognition-db.json')"));assert(code.includes('fineSilhouette'));assert(code.includes("features=crops.map(c=>visualFeature(c,false,side))"));assert(code.includes('ocrVisualCrops'));assert(code.includes('visualRankings=uniqueVisualRankings'));assert(code.includes('saveLearnedReferences'));
const recognition=JSON.parse(fs.readFileSync(root+'/dist/recognition-db.json')),baseRefs=recognition.references.filter(x=>x.source==='현재 목록 기본 이미지'),screenRefs=recognition.references.filter(x=>x.source==='사용자 제공 챔피언스 화면');assert.equal(recognition.schemaVersion,2);assert.equal(baseRefs.length,235);assert.equal(new Set(baseRefs.map(x=>x.name)).size,235);assert.equal(screenRefs.length,36);assert(screenRefs.some(x=>x.name==='워시로토무'));for(const name of ['로토무','워시로토무','히트로토무','커트로토무','프로스트로토무','스핀로토무'])assert(baseRefs.some(x=>x.name===name));for(const ref of recognition.references){assert(currentPokemon.has(ref.name));assert(fs.statSync(root+'/dist/'+ref.src).size>100)}
const html=fs.readFileSync(root+'/dist/index.html','utf8'),manifest=JSON.parse(fs.readFileSync(root+'/dist/manifest.webmanifest')),version=JSON.parse(fs.readFileSync(root+'/dist/version.json'));assert(html.includes('<title>포챔스 달콤아 봇</title>'));assert(html.includes('assets/dalkoma-logo-192.png'));assert(html.includes('dalkoma-favicon.png?v=15'));assert(html.includes('<strong class="appversion">v15</strong>'));assert.equal(manifest.name,'포챔스 달콤아 봇');assert.equal(version.version,'v15');assert.equal(version.name,'포챔스 달콤아 봇');for(const icon of ['dalkoma-favicon.png','dalkoma-apple-touch.png','dalkoma-logo-192.png','dalkoma-logo-512.png'])assert(fs.statSync(root+'/dist/assets/'+icon).size>100);assert(code.includes("['PokeAPI 보조','현재 목록 기본 이미지']"));
...
const spriteSync=fs.readFileSync(root+'/scripts/sync_pokeapi_sprites.py','utf8');assert(spriteSync.includes('pokemon/shiny'));assert(spriteSync.includes('pokemon/other/home/shiny'));assert(spriteSync.includes('sprite-home-map.json'));assert(spriteSync.includes('RECOGNITION_DB'));assert(spriteSync.includes('현재 목록 기본 이미지'));
