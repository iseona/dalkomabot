...
{const rows=D.modes.single,matrix=teamDefenseMatrix(rows,['한카리아스']);assert.equal(matrix.length,18);assert.equal(matrix.find(x=>x.type==='얼음').cells[0].value,4);const delta=coverageDelta(rows.find(x=>x.name==='메타그로스'),rows,['한카리아스']);assert(delta.repaired.includes('얼음'));assert(delta.afterScore>delta.beforeScore)}
