...
try{
 ...
 const atkEvEditor=page.locator('.calc-attack .stat-editor').first();assert.equal(await atkEvEditor.count(),1);const attackEv=atkEvEditor.locator('[data-stat="1"]');await attackEv.fill('30');await attackEv.locator('xpath=..').locator('[data-step="1"]').click();assert.equal(await attackEv.inputValue(),'31');assert.match(await page.locator('#calcAtkEvs').inputValue(),/^\d+,31,/);await attackEv.fill('32');const speedEv=atkEvEditor.locator('[data-stat="5"]');await speedEv.fill('32');assert(Number((await page.locator('#calcAtkEvs').inputValue()).split(',').reduce((a,b)=>a+Number(b),0))<=66,'노력치 합계 제한 실패');
 await page.locator('#themeToggle').click();assert.equal(await page.locator('html').getAttribute('data-theme'),'dark');await page.reload({waitUntil:'domcontentloaded'});assert.equal(await page.locator('html').getAttribute('data-theme'),'dark');assert.deepEqual(errors,[]);console.log('PASS: stat steppers, EV cap, and persistent dark mode.');
