...
p=argparse.ArgumentParser();p.add_argument('--region',default='ap-northeast-2');p.add_argument('--stack',default='champions-party-lab');p.add_argument('--discord-public-key',default='');p.add_argument('--openai-model',default='gpt-5.2');p.add_argument('--max-daily-recognition-requests',type=int,default=100);p.add_argument('--enable-collector-schedule',action='store_true',help='Enable daily collection after manual verification.');p.add_argument('--run-collector',action='store_true',help='Invoke collector synchronously and verify its published object.');p.add_argument('--google-client-id',default=os.environ.get('GOOGLE_CLIENT_ID',''));p.add_argument('--cognito-domain-prefix',default=os.environ.get('COGNITO_DOMAIN_PREFIX',''));a=p.parse_args()
...
if not a.google_client_id or not google_secret or not a.cognito_domain_prefix:raise SystemExit('Set GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and COGNITO_DOMAIN_PREFIX before deploying.')
aws('sts','get-caller-identity')
aws('cloudformation','deploy','--region',a.region,'--stack-name',a.stack,'--template-file',str(ROOT/'aws/template.json'),'--capabilities','CAPABILITY_IAM','--parameter-overrides','DiscordPublicKey='+a.discord_public_key,'OpenAIModel='+a.openai_model,'MaxDailyRecognitionRequests='+str(a.max_daily_recognition_requests),'EnableCollectorSchedule='+str(a.enable_collector_schedule).lower(),'GoogleClientId='+a.google_client_id,'GoogleClientSecret='+google_secret,'CognitoDomainPrefix='+a.cognito_domain_prefix)
