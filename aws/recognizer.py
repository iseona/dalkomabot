...
def build_request(images,prompt):
 ...
 return {'model':MODEL,'store':False,'max_output_tokens':1200,'input':[{'role':'user','content':content}],'text':{'format':{'type':'json_schema','name':'champions_screen','strict':True,'schema':SCHEMA}}}
...
def handler(event,context):
 try:
  ...
  prompt=f'포켓몬 챔피언스 {mode} 화면이다. 화면에 보이는 값만 기록하고 추측하지 마라. 응답은 항상 schemaVersion, kind, slots, sides를 모두 포함한다. party는 slots에 2열 3행을 위에서 아래, 왼쪽에서 오른쪽 슬롯 1~6으로 포켓몬명, 도구, 특성, 성격, 기술, HABCDS 배분을 기록하고 sides.mine과 sides.opp는 빈 배열로 둔다. lead는 slots를 빈 배열로 두고 왼쪽 내 파티를 sides.mine, 오른쪽 상대 파티를 sides.opp에 각각 위에서 아래 슬롯 1~6으로 기록하며 포켓몬명만 읽고 나머지는 null 또는 빈 배열로 둔다. 별명만 보이거나 불확실한 값은 null 또는 빈 배열로 남기고 notes에 이유를 적어라.'
