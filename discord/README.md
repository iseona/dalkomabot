...
| `/도감 분류:포켓몬 검색어:한카리아스` | 포켓몬·기술·특성·도구 이름/효과 검색과 상세 정보 |
...
초기 응답 제한을 고려해 계산은 동기적으로 수행합니다. 상세 통계 `data.json`과 공개 파티 `opendata.json`은 명령 요청이 있을 때 자체 웹앱의 검증된 캐시를 확인하며, 같은 Lambda 인스턴스에서는 한 시간 동안 다시 조회하지 않습니다. 따라서 웹 데이터만 갱신해도 최대 약 한 시간 안에 봇에 반영됩니다. 두 파일은 병렬로 최대 0.9초 동안 조회하며 원본 데이터 제공 사이트에 명령마다 요청하지 않습니다. 자체 데이터 조회나 검증이 실패하면 Lambda 패키지의 마지막 정상 스냅샷을 사용하고 응답에 데이터 기준일을 표시합니다.
